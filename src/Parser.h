...
public:
  ...
  [[nodiscard]] std::unordered_map<std::pair<std::string, std::string>, Route, pair_hash>
  getRoutes(); // Key is (route_id, direction_id)
  [[nodiscard]] std::unordered_map<std::string, Trip> getTrips();

  [[nodiscard]] std::unordered_map<std::pair<std::string, std::string>, StopTime, pair_hash>
  getStopTimes(); // key is (trip_id, stop_id)
